      ...
      <div className="container-x py-section">
        ...
        <div className="mt-12 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {packages.slice(0, 6).map((p, i) => (
            <Link key={`${p.name}-${i}`} to="/contact" className={`group flex flex-col rounded-2xl border bg-fam-card p-6 transition-all hover:shadow-md ${p.popular ? 'border-fam-accent' : 'border-fam-hairline hover:border-fam-accent'}`}>
