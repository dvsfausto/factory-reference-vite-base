          ...
          {memberships.map((m, i) => (
            <div key={`${m.name}-${i}`} className={`grid grid-cols-1 items-center gap-4 border-b border-fam-hairline p-6 last:border-0 md:grid-cols-12 md:gap-6 ${m.highlighted ? 'bg-fam-accent-soft' : 'bg-fam-card'}`}>
              ...
              <div className="md:col-span-2 md:text-right">
                <Link to="/contact" className="inline-flex h-11 items-center gap-1.5 rounded-xl bg-cta px-5 font-display text-sm font-semibold text-cta-foreground transition-opacity hover:opacity-90">
