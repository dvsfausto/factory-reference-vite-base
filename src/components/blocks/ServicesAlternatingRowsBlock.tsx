          ...
          {rows.map((s, i) => (
            <div key={s.slug} className="grid items-center gap-10 lg:grid-cols-2 lg:gap-16">
              ...
              <div className={i % 2 === 1 ? 'lg:order-1' : ''}>
                <span className="font-display text-sm font-semibold text-fam-accent-text">
                  {String(i + 1).padStart(2, '0')}
                </span>
                <h3 className="mt-3 font-display text-3xl font-semibold tracking-tight text-fam-ink sm:text-4xl">
                  {s.displayName}
                </h3>
                <p className="mt-4 max-w-md text-lg leading-relaxed text-fam-ink-muted">{s.short}</p>
                <Link
                  to="/services/$slug"
                  params={{ slug: s.slug }}
