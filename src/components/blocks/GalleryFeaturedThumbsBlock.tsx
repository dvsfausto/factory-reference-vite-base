        ...
        <div className="mt-12">
          <figure className="overflow-hidden rounded-3xl border border-fam-hairline bg-fam-card">
            ...
            <figcaption className="flex items-center justify-between gap-3 p-6">
              <span className="font-display text-lg font-semibold text-fam-ink">{featured.title}</span>
              {featured.caption && <span className="text-sm italic text-fam-ink-faint">{featured.caption}</span>}
            </figcaption>
          </figure>
            ...
            {projects.map((p, i) => (
              <button
                ...
                className={`aspect-square overflow-hidden rounded-xl border transition-all ${
                  i === active ? 'border-fam-accent ring-2 ring-fam-accent' : 'border-fam-hairline opacity-80 hover:opacity-100'
                }`}
              >
