        ...
        <div className="mt-12 grid grid-cols-1 gap-6 md:grid-cols-3">
          {promos.slice(0, 3).map((p, i) => (
            <div key={`${p.title}-${i}`} className="flex flex-col rounded-2xl border border-fam-hairline bg-fam-card p-7">
              ...
              {p.code && <span className="mt-5 w-fit rounded-lg border border-dashed border-fam-accent px-3 py-1.5 font-display text-sm font-semibold tracking-wider text-fam-accent-text-strong">{tr('blk.code')} {p.code}</span>}
              {p.expires && <span className="mt-3 text-xs text-fam-ink-faint">{tr('blk.ends')} {p.expires}</span>}
              <Link to="/contact" className="mt-6 inline-flex h-11 items-center justify-center rounded-xl bg-cta px-5 font-display text-sm font-semibold text-cta-foreground transition-[filter] hover:brightness-(--hov-shade)">{tr('blk.claimOffer')}</Link>
            </div>
          ))}
        </div>
      </div>
